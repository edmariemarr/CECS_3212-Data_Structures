...
protected:
	...
	BinaryNode<ItemType>* removeLeftmostNode(BinaryNode<ItemType>* nodePtr, const ItemType& inorderSuccesssor);
...
public:
	...
	void attachLeftSubtree(BinaryNodeTree<ItemType>* leftTreePtr) const
...
template < class ItemType>
BinaryNodeTree<ItemType>::BinaryNodeTree(const ItemType& rootItem,
	const BinaryNodeTree<ItemType>* leftTreePtr, const BinaryNodeTree<ItemType>* rightTreePtr)
{
...
throw(PrecondViolatedExcep)
{
	BinaryNode<ItemType>* rightTreePtr;
	if (isEmpty())
		throw PrecondViolatedExcep("TreeException: Empty tree");
	else
		...
		{
			rightTreePtr = new BinaryNode<ItemType>(anEntry, nullptr,
				nullptr);
			rootPtr->setRightChildPtr(rightTreePtr);
			if (rootPtr->getRightChildPtr() == nullptr)
				throw PrecondViolatedExcep("TreeException: Can not allocate memory");
		} //end if
}// end attachLeft
...
/*
template < class ItemType>
int BinaryNodeTree<ItemType>::precedence(char ch) {
	if (ch == '+' || ch == '-') {
		return 1;
	}
	else if (ch == '*' || ch == '/') {
...
template < class ItemType>
bool BinaryNodeTree<ItemType>::isOperand(char ch) {
...
template < class ItemType>
bool BinaryNodeTree<ItemType>::isOperator(char ch) {
	if (ch == '+' || ch == '-' || ch == '*' || ch == '/' || ch == '^')
		return true;
	else
		return false;
